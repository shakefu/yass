...
> from scratch to absorb these language changes. Resolved entries describe the
> spec-state as of the round in which they were fixed; the durable fixes live in
> `yass.yass.yaml` / `yass-reference` / `GUIDANCE`, not in the example set.
...
| `unreachable-codes` | repeated | open | Several error codes unreachable by design; mark (SHADOWED-BY/UNREACHABLE-WHEN) or remove. → yass.yass.yaml (language-level marker; the former example error registry was removed with `spec/cli.*`) |
